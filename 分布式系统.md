...
* 原子提交
  ...
  突然事务涉及的一台服务器出现错误了，我们需要能从这种场景恢复。所以，哪怕事务涉及的机器只有部分还在运行，我们需要具备能够从部分故障中恢复的能力。

* 定义什么是正确

  Serializable： 可序列化 

  a, b = 10, 10 T1 a++ b--          T2 print a print b

  要么T1 T2 输出 11, 9 要么 T2 T1 输出 10,10   除此之外的结果都是错误